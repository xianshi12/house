<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('fangyuanxinxi','首页总数')">
				<div class="left">
			...
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('kanfangyuyue','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{kanfangyuyueCount}}</div>
					<div class="name">看房预约总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('hetongqianding','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-qita3"></span>
				</div>
				<div class="right">
					<div class="num">{{hetongqiandingCount}}</div>
					<div class="name">合同签订总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<el-dialog :visible.sync="newsVisible" title="售房资讯" :append-to-body="true" width="60%">
		...
		<!-- 4 -->
		<div class="type4">
			<div id="fangyuanxinxiChart1" class="echarts1 animate__animated" v-if="isAuth('fangyuanxinxi','首页统计')"></div>
			<div id="kanfangyuyueChart1" class="echarts2 animate__animated" v-if="isAuth('kanfangyuyue','首页统计')"></div>
			<div id="hetongqiandingChart1" class="echarts3 animate__animated" v-if="isAuth('hetongqianding','首页统计')"></div>
			<div id="hetongqiandingChart2" class="echarts4 animate__animated" v-if="isAuth('hetongqianding','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getkanfangyuyueCount();
		if(this.isAuth('kanfangyuyue','首页统计')){
			this.kanfangyuyueChat1();
		}
		this.gethetongqiandingCount();
		if(this.isAuth('hetongqianding','首页统计')){
			this.hetongqiandingChat1();
		}
		if(this.isAuth('hetongqianding','首页统计')){
			this.hetongqiandingChat2();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'kanfangyuyueChart1',css:'animate__fadeInUp'},
				{id:'hetongqiandingChart1',css:'animate__fadeInUp'},
				{id:'hetongqiandingChart2',css:'animate__fadeInUp'},
			]
...
// 1234 饼
		...
		getkanfangyuyueCount() {
			this.$http({
				url: `kanfangyuyue/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.kanfangyuyueCount = data.data
				}
			})
		},
// 1234 折滑面
		kanfangyuyueChat1() {
			this.$nextTick(()=>{
				...
				var kanfangyuyueChart1 = echarts.init(document.getElementById("kanfangyuyueChart1"),'macarons');
				this.$http({
					url: "kanfangyuyue/group/dengjishijian",
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						let res = data.data;
						...
						// 使用刚指定的配置项和数据显示图表。
						kanfangyuyueChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							kanfangyuyueChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		gethetongqiandingCount() {
			this.$http({
				url: `hetongqianding/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.hetongqiandingCount = data.data
				}
			})
		},
// 1234 竖
		hetongqiandingChat1() {
			this.$nextTick(()=>{
				...
				var hetongqiandingChart1 = echarts.init(document.getElementById("hetongqiandingChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						hetongqiandingChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							hetongqiandingChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		hetongqiandingChat2() {
			this.$nextTick(()=>{
				...
				var hetongqiandingChart2 = echarts.init(document.getElementById("hetongqiandingChart2"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						hetongqiandingChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							hetongqiandingChart2.resize();
						};
					}else{
						this.$message({
							message: data.msg,
...
<style lang="scss" scoped>
	.home-content {
		padding: 10px 30px;
		margin: 0  0 0 40px;
		background: url(http://codegen.caihongy.cn/20250408/c814c1f017af4f4b930d27c29241eed9.png) no-repeat center top / cover;
		display: flex;
		width: 100%;
		min-height: 100vh;
