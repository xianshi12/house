<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'" label="订单编号" prop="orderNo" >
					<el-input v-model="ruleForm.dingdanbianhao" placeholder="订单编号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.dingdanbianhao" label="订单编号" prop="orderNo" >
					<el-input v-model="ruleForm.dingdanbianhao" placeholder="订单编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="意向金占比" prop="intentDepositRatio" >
					<el-input-number v-model="ruleForm.yixiangjinzhanbi" placeholder="意向金占比" :disabled="ro.yixiangjinzhanbi"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="意向金占比" prop="intentDepositRatio" >
					<el-input v-model="ruleForm.yixiangjinzhanbi" placeholder="意向金占比" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="意向定金" prop="intentDeposit" >
					<el-input v-model="intentDeposit" placeholder="意向定金" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.yixiangdingjin" label="意向定金" prop="intentDeposit" >
...
<script>
	...
	export default {
		...
		computed: {
			...
			yixiangdingjin:{
				get: function () {
					return 1*this.ruleForm.chushoujiage*this.ruleForm.yixiangjinzhanbi
				}
			},
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='intentDepositRatio'){
							this.ruleForm.yixiangjinzhanbi = obj[o];
							this.ro.yixiangjinzhanbi = true;
							continue;
						}
						if(o=='intentDeposit'){
					...
					this.ruleForm.yixiangjinzhanbi = Number('0.1'); 				}
				// 获取用户信息
				this.$http({
			...
			async onSubmit() {
					...
					this.ruleForm.yixiangdingjin = this.yixiangdingjin
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yixiangjinzhifu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										duration: 1500,
										onClose: () => {
											if(this.isAuth('yixiangjinzhifu','支付')&&this.type=='cross') {
												this.$confirm('是否跳转支付？').then(_ => {
													this.parent.showFlag = true;
													this.parent.addOrUpdateFlag = false;
													this.parent.yixiangjinzhifuCrossAddOrUpdateFlag = false;
													this.$router.push('/yixiangjinzhifu')
												}).catch(_ => {
													this.parent.showFlag = true;
													this.parent.addOrUpdateFlag = false;
													this.parent.yixiangjinzhifuCrossAddOrUpdateFlag = false;
