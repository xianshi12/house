		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yixiangjinzhifu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#FFFFFF","height":"34px"}'></span>
						发布
					</el-button>
					<el-button class="del" v-if="isAuth('yixiangjinzhifu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yixiangjinzhifu','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"17px","color":"#000"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('yixiangjinzhifu','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="intentDepositRatio"
						label="意向金占比">
						<template slot-scope="scope">
							{{scope.row.yixiangjinzhanbi}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="intentDeposit"
					...
					<el-table-column :resizable='true' :sortable='true' prop="isPaid" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('yixiangjinzhifu','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yixiangjinzhifu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yixiangjinzhifu','合同签订')" @click="hetongqiandingCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								合同签订
							</el-button>
							<el-button class="edit" v-if=" isAuth('yixiangjinzhifu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yixiangjinzhifu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
...
<script>
	...
	import hetongqiandingCrossAddOrUpdate from "../contractSigning/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		...
		methods: {
			...
			hetongqiandingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','yixiangjinzhifu');
			...
			payHandler(row){
				this.$storage.set('paytable','yixiangjinzhifu');
			...
			getDataList() {
				...
				if(this.searchForm.ispay!='' && this.searchForm.ispay!=undefined){
					params['isPaid'] = this.searchForm.ispay
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "yixiangjinzhifu/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
