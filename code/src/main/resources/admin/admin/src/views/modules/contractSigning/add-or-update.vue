	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="订单编号" prop="orderNo" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="意向定金" prop="intentDeposit" >
					<el-input-number v-model="ruleForm.yixiangdingjin" placeholder="意向定金" :disabled="ro.yixiangdingjin"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="意向定金" prop="intentDeposit" >
				...
				<el-form-item v-if="ruleForm.yonghuhetong" label="用户回传合同" prop="userContract" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.yonghuhetong)">
						<span class="icon iconfont icon-xiazai6"></span>
						下载
					</el-button>
				</el-form-item>
				<el-form-item v-else label="用户回传合同" prop="userContract" >
					<el-button class="unBtn" type="text" size="small">
						<span class="icon iconfont icon-xihuan"></span>
						暂无
					</el-button>
	...
	export default {
		...
		methods: {
			...
			async onSubmit() {
					...
					if(this.ruleForm.yonghuhetong!=null) {
						this.ruleForm.yonghuhetong = this.ruleForm.yonghuhetong.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `hetongqianding/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.hetongqiandingCrossAddOrUpdateFlag = false;
											this.parent.search();
