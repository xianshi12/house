<template>
	<div class="main-content" :style='{"padding":"20px 30px","margin":"0 0 0 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","flexWrap":"wrap","background":"none","display":"flex"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">楼层</label>
						<el-input v-model="searchForm.louceng" placeholder="楼层" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">厅室</label>
						<el-input v-model="searchForm.tingshi" placeholder="厅室" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">地段</label>
						<el-input v-model="searchForm.diduan" placeholder="地段" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">位置</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('fangyuanxinxi','房屋类型占比')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"17px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' prop="saleContract" label="出售合同">
						<template slot-scope="scope">
							<el-button v-if="scope.row.chushouhetong" type="text" size="small" @click="download(scope.row.chushouhetong)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('fangyuanxinxi','预约看房')" @click="kanfangyuyueCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								预约看房
							</el-button>
							<el-button class="btn8" v-if="isAuth('fangyuanxinxi','意向下单')" @click="yixiangjinzhifuCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								意向下单
							</el-button>
		...
		<kanfangyuyue-cross-add-or-update v-if="kanfangyuyueCrossAddOrUpdateFlag" :parent="this" ref="kanfangyuyueCrossaddOrUpdate"></kanfangyuyue-cross-add-or-update>
		<yixiangjinzhifu-cross-add-or-update v-if="yixiangjinzhifuCrossAddOrUpdateFlag" :parent="this" ref="yixiangjinzhifuCrossaddOrUpdate"></yixiangjinzhifu-cross-add-or-update>
...
<script>
	...
	import kanfangyuyueCrossAddOrUpdate from "../viewingReservation/add-or-update";
	import yixiangjinzhifuCrossAddOrUpdate from "../intentDepositPayment/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				kanfangyuyueCrossAddOrUpdateFlag: false,
				yixiangjinzhifuCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			kanfangyuyueCrossAddOrUpdate,
			yixiangjinzhifuCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			kanfangyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.kanfangyuyueCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kanfangyuyueCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kanfangyuyueCrossaddOrUpdate.init(row.id,type);
				});
			},
			yixiangjinzhifuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yixiangjinzhifuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fangyuanxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yixiangjinzhifuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yixiangjinzhifuCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			getDataList() {
				...
				if(this.searchForm.fangwuleixing!='' && this.searchForm.fangwuleixing!=undefined){
					params['houseType'] = this.searchForm.fangwuleixing
				}
 				if(this.searchForm.louceng!='' && this.searchForm.louceng!=undefined){
					params['floor'] = '%' + this.searchForm.louceng + '%'
				}
 				if(this.searchForm.tingshi!='' && this.searchForm.tingshi!=undefined){
					params['rooms'] = '%' + this.searchForm.tingshi + '%'
				}
 				if(this.searchForm.diduan!='' && this.searchForm.diduan!=undefined){
					params['district'] = '%' + this.searchForm.diduan + '%'
				}
 				if(this.searchForm.weizhi!='' && this.searchForm.weizhi!=undefined){
					params['address'] = '%' + this.searchForm.weizhi + '%'
				}
