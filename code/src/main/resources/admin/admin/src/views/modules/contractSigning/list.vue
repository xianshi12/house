<template>
	<div class="main-content" :style='{"padding":"20px 30px","margin":"0 0 0 30px","fontSize":"15px"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">订单编号</label>
						<el-input v-model="searchForm.dingdanbianhao" placeholder="订单编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">房屋名称</label>
					...
					<div :style='{"margin":"0 10px 0 0","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">是否通过</label>
						<el-select clearable v-model="searchForm.sfsh" placeholder="是否通过">
							<el-option v-for="(item,index) in sfshOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">是否支付</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('hetongqianding','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#FFFFFF","height":"34px"}'></span>
						发布
					</el-button>
					<el-button class="del" v-if="isAuth('hetongqianding','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('hetongqianding','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('hetongqianding','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						批量支付
					</el-button>
					<el-button class="btn18" v-if="isAuth('hetongqianding','房屋出售收入情况')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						房屋出售收入情况
					</el-button>
					<el-button class="btn18" v-if="isAuth('hetongqianding','交易房屋量情况')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						交易房屋量情况
					</el-button>
				</el-row>
			</el-form>
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"17px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="propertyImage" width="200" label="房屋图片">
						<template slot-scope="scope">
					...
					<el-table-column :resizable='true' prop="contract" label="合同">
						<template slot-scope="scope">
							<el-button v-if="scope.row.hetong" type="text" size="small" @click="download(scope.row.hetong)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' prop="userContract" label="用户回传合同">
						<template slot-scope="scope">
							<el-button v-if="scope.row.yonghuhetong" type="text" size="small" @click="download(scope.row.yonghuhetong)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="isPaid" label="是否支付">
						<template slot-scope="scope">
							<span style="margin-right:10px">{{scope.row.ispay=='已支付'?'已支付':'未支付'}}</span>
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('hetongqianding','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="auditReply" label="审核回复" show-overflow-tooltip>
						<template slot-scope="scope">
							<div style="white-space: nowrap;">{{scope.row.shhf}}</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="auditStatus" label="审核状态">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('hetongqianding','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('hetongqianding','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('hetongqianding','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<el-dialog
			...
			width="800">
			<div id="chushoujiageChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="hetongChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			payHandler(row){
				...
				this.$storage.set('paytable','hetongqianding');
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var chushoujiageChart1 = echarts.init(document.getElementById("chushoujiageChart1"),'macarons');
					this.$http({
						url: `hetongqianding/value/dengjishijian/chushoujiage`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							chushoujiageChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								chushoujiageChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var hetongChart2 = echarts.init(document.getElementById("hetongChart2"),'macarons');
					this.$http({
						url: "hetongqianding/group/hetong",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.dingdanbianhao!='' && this.searchForm.dingdanbianhao!=undefined){
					params['orderNo'] = '%' + this.searchForm.dingdanbianhao + '%'
				}
 				if(this.searchForm.fangwumingcheng!='' && this.searchForm.fangwumingcheng!=undefined){
					params['propertyName'] = '%' + this.searchForm.fangwumingcheng + '%'
				}
				if(this.searchForm.sfsh!='' && this.searchForm.sfsh!=undefined){
					params['auditStatus'] = this.searchForm.sfsh
				}
			...
			shBatchDialog(){
				...
				for(let x in this.dataListSelections){
					...
					if(!this.dataListSelections[x].yonghuhetong){
						this.$message.error('用户尚未回传合同，不能审核');
						this.batchIds = []
						return false
					}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "hetongqianding/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
