	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.touxiang" label="头像" prop="avatar" >
					<file-upload
						tip="点击上传头像"
						action="file/upload"
						:limit="3"
				...
				<el-form-item class="upload" v-else-if="ruleForm.touxiang" label="头像" prop="avatar" >
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `kaifashang/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.kaifashangCrossAddOrUpdateFlag = false;
											this.parent.search();
