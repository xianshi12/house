		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.louceng" placeholder="楼层" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">厅室：</div>
					<el-input v-model="formSearch.tingshi" placeholder="厅室" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">地段：</div>
					<el-input v-model="formSearch.diduan" placeholder="地段" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-button class="list-add-btn" v-if="btnAuth('fangyuanxinxi','新增')" type="primary" @click="add('/index/fangyuanxinxiAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="avatar" @click.stop="imgPreView(item.fangwutupian)" v-if="item.fangwutupian && item.fangwutupian.substr(0,4)=='http'&& item.fangwutupian.split(',w').length>1" :src="item.fangwutupian" />
						<img class="avatar" @click.stop="imgPreView(item.fangwutupian.split(',')[0])" v-else-if="item.fangwutupian && item.fangwutupian.substr(0,4)=='http'" :src="item.fangwutupian.split(',')[0]" />
						<img class="avatar" @click.stop="imgPreView(baseUrl + (item.fangwutupian?item.fangwutupian.split(',')[0]:''))" v-else :src="baseUrl + (item.fangwutupian?item.fangwutupian.split(',')[0]:'')" />
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('fangwuleixing/list',{params: {sort: 'houseType',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
						this.fenlei = res.data.data.list
					}
				});
				this.feileiColumn = 'houseType'
			},
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.louceng != '') searchWhere.louceng = '%' + this.formSearch.louceng + '%';
				if (this.formSearch.tingshi != '') searchWhere.tingshi = '%' + this.formSearch.tingshi + '%';
				if (this.formSearch.diduan != '') searchWhere.diduan = '%' + this.formSearch.diduan + '%';
				...
				this.$http.get(`fangyuanxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/fangyuanxinxiDetail', query: params});
			},
