	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/fangyuanxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.weizhi}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">出售合同</div>
						<el-button class="uploadBtn" @click="preview(detail.chushouhetong )">在线预览</el-button>
						<el-button class="uploadBtn" @click="download(detail.chushouhetong )">点击下载</el-button>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('fangyuanxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('fangyuanxinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('fangyuanxinxi','预约看房')" 
							@click="onAcross('kanfangyuyue','','','','')" type="warning">
							预约看房
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('fangyuanxinxi','意向下单')" 
							@click="onAcross('yixiangjinzhifu','','','','')" type="warning">
							意向下单
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="详情" name="first">
					<div class="ql-snow ql-editor" v-html="detail.xiangqing"></div>
				</el-tab-pane>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('fangyuanxinxi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'fangyuanxinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/fangyuanxinxi', query: params});
				}
			},
			// 下载
			preview(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussfangyuanxinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussfangyuanxinxi/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussfangyuanxinxi/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussfangyuanxinxi/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 修改
			editClick(){
				this.$router.push(`/index/fangyuanxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此房源信息？') .then(_ => {
					this.$http.post('fangyuanxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
