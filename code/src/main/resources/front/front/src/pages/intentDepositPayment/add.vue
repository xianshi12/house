<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="意向金占比" prop="intentDepositRatio">
				<el-input-number v-model="ruleForm.yixiangjinzhanbi" placeholder="意向金占比" :disabled="true ||ro.yixiangjinzhanbi"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item" label="意向定金" prop="intentDeposit">
				<el-input v-model="intentDeposit" placeholder="意向定金" disabled></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`yixiangjinzhifu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yixiangjinzhifu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
									duration: 1500,
									onClose: () => {
										if(this.isBackAuth('yixiangjinzhifu','支付')&&this.type=='cross'){
											this.$confirm('是否跳转支付？') .then(_ => {
												let jumpParams = {
													id: res.data.data,
													centerType: 1
												}
												this.$router.push({path: '/index/yixiangjinzhifuDetail', query: jumpParams});
											}).catch(_ => {
												this.$router.go(-1);
											});
										}else {
