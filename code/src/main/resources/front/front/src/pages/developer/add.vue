<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="性别" prop="gender">
			...
			<el-form-item class="add-item" label="头像" v-if="type!='cross' || (type=='cross' && !ro.touxiang)" prop="avatar">
				<file-upload
					tip="点击上传头像"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="头像" prop="avatar">
	...
	export default {
		data() {
			return {
				...
				rules: {
					kaifashanghao: [
						{ required: true, message: '开发商号不能为空', trigger: 'blur' },
					],
					mima: [
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`kaifashang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`kaifashang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
