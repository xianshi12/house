	...
	<div class="main-containers">
		<div class="body-containers">
			<div class="top-container">
				<!-- info -->
				<div class="top_title">
					<span @click="goMenu('/index/home')">房屋出售系统</span>
				</div>
	...
	methods: {
		...
		async getSession() {
			await this.$http.get(`${localStorage.getItem('UserTableName')}/session`, {emulateJSON: true}).then(async res => {
				if (res.data.code == 0) {
					if(localStorage.getItem('UserTableName')== 'user') {
						localStorage.setItem('username', res.data.data.zhanghao);
					}
					if(localStorage.getItem('UserTableName')== 'developer') {
						localStorage.setItem('username', res.data.data.kaifashanghao);
					}
